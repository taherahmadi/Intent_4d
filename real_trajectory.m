...
t = smoothdata(txy(:,1)); % problematic to smooth time?
len = length(t)
for i=1:len-1
...
%% plot
[x_prime,y_prime] = pol2cart(thetav(1:end-1), a);
xy_prime = [x_prime',y_prime'];

figure;
hold on;
scatter(xy_data(:,1),xy_data(:,2),'.');
quiver(xy_data(1:end-2,1),xy_data(1:end-2,2),xy_prime(:,1),xy_prime(:,2), 'MaxHeadSize',0.001);
xlabel('x');
ylabel('y');
%